...
from models.swin_encoder_tf1 import SwinEncoderTF
...
class DPF():
    ...
    def build_modules(self, min_obs_likelihood, proposer_keep_ratio):
        ...
        # Swin Transformer encoder — hierarchical windowed attention
        self.encoder = SwinEncoderTF(
    ...
    def transform_particles_as_input(self, particles, means, stds):
        return tf.concat([
                   (particles[:, :, :2] - means['s'][:, :, :2]) / stds['s'][:, :, :2],
                   tf.cos(particles[:, :, 2:3]),
                   tf.sin(particles[:, :, 2:3])],
                  axis=-1)
    ...
    def connect_modules(self, means, stds, state_mins, state_maxs, state_step_sizes):
        ...
        # Swin encoder: flatten time dim, encode, reshape back
        ...
        def loop(particles, particle_probs, particle_list, particle_probs_list, additional_probs_list, i):
            ...
            if self.propose_ratio < 1.0:
                ...
                markers = random_offset[:, None] + basic_markers[None, :]
    ...
    def plot_particle_filter(self, sess, batch, particle_list,
        ...
        pred, s_particle_list, s_particle_probs_list = self.predict(sess, batch, num_particles, return_particles=True)
        num_steps = 20
        for s in range(1):
            ...
            for i in range(num_steps):
                ...
                if i < num_steps - 1:
                    ...
                    ax.plot(pred[s, :num_steps, 0], pred[s, :num_steps, 1], '-', linewidth=0.6, color=colors['pf_ind_e2e'])
                    ax.plot(batch['s'][s, :1, 0], batch['s'][s, :1, 1], '.', linewidth=0.6, color='red', markersize=3)
                    ax.plot(pred[s, :1, 0], pred[s, :1, 1], '.', linewidth=0.6, markersize=3, color=colors['pf_ind_e2e'])
